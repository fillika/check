...
class Comparator implements IComparator {
    ...
    get not() {
        ...
        return {
            ...
            toBeFalsy() {
                if (self.param === false) error(`expect(${self.param}).not.toBeFalsy()`);
            },
            toBeTruthy() {
                if (self.param === true) error(`expect(${self.param}).not.toBeTruthy()`);
            },
            toBeUndefined() {
                if (self.param === undefined) error(`expect(${self.param}).not.toBeUndefined()`);
            },
        }
    }
    ...
    toBeUndefined() {
        if (this.param !== undefined) error(`expect(${this.param}).toBeUndefined()`);
    }
}
